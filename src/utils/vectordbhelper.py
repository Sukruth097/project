...
class VectorDatabaseHelper:
    def __init__(self):
        logger.debug("Initializing VectorDatabaseHelper")
        ...
        logger.debug("LLMHelper and embedding model initialized")
    ...
    def define_properties(self):
        ...
        try:
            ...
            logger.debug(f"Properties defined: {properties}")
            return properties
    ...
    def ingest_text_data(self, collection, text_data):
        logger.debug("Ingesting text data")
        try:
            with collection.batch.dynamic() as batch:
                for text in tqdm(text_data, desc="Ingesting text data"):
                    logger.debug(f"Ingesting text: {text}")
                    vector = self.embedding_model(text['text'])
                    logger.debug(f"Generated vector: {vector}")
                    text_obj = {
    ...
    def ingest_image_data(self, collection, image_data):
        logger.debug("Ingesting image data")
        try:
            with collection.batch.dynamic() as batch:
                for image in tqdm(image_data, desc="Ingesting image data"):
                    logger.debug(f"Ingesting image: {image}")
                    ...
                    logger.debug("Image object added to batch")
        except Exception as e:
    ...
    def ingest_table_data(self, collection, table_data):
        logger.debug("Ingesting table data")
        try:
            with collection.batch.dynamic() as batch:
                for table in tqdm(table_data, desc="Ingesting table data"):
                    logger.debug(f"Ingesting table: {table}")
    ...
    def ingest_all_data(self, collection_name, text_data, image_data, table_data):
        logger.debug(f"Ingesting all data into collection: {collection_name}")
        try:
